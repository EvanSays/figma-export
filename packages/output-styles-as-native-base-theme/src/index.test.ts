...
describe('style output as scss', () => {
    ...
    it('should not print anything if style is not visible', async () => {
        ...
        expect(writeFileSync).to.be.calledWithMatch('/output-folder/_variables.json', '');
    });

    it('should be able to change the filename and the output folder', async () => {
        await outputter({
            output: 'output-folder',
        ...
        expect(writeFileSync).to.be.calledOnce;
        expect(writeFileSync).to.be.calledWithMatch('/output-folder/_figma-styles.json');
    });
